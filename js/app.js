...
// ============================================================
//  🔍 WORD SCAN — Rà soát từ
// ============================================================
const SCAN_PAGE_SIZE = 30;
let _scanPage = 0;
let _scanDraft = {}; // { word: true/false } — trạng thái hidden tạm thời trong session

function openWordScan() {
  if (!words.length) { showToast('error', '⚠ Chưa có từ nào.'); return; }
  // Khởi tạo draft từ trạng thái hiện tại
  _scanDraft = {};
  words.forEach(w => { _scanDraft[w.word] = !!w.hidden; });
  _scanPage = 0;
  document.getElementById('wordScanOverlay').classList.add('show');
  _renderScanPage();
}

function closeWordScan() {
  document.getElementById('wordScanOverlay').classList.remove('show');
}

function _renderScanPage() {
  const all = words; // hiển thị tất cả từ
  const totalPages = Math.ceil(all.length / SCAN_PAGE_SIZE);
  const start = _scanPage * SCAN_PAGE_SIZE;
  const pageWords = all.slice(start, start + SCAN_PAGE_SIZE);

  // Progress bar
  const hiddenCount = Object.values(_scanDraft).filter(Boolean).length;
  const pct = all.length ? Math.round(hiddenCount / all.length * 100) : 0;
  document.getElementById('wordScanProgressBar').style.width = pct + '%';

  // Page info
  document.getElementById('wordScanPageInfo').textContent =
    `Trang ${_scanPage + 1} / ${totalPages}`;

  // Nav buttons
  document.getElementById('wordScanPrev').disabled = _scanPage === 0;
  document.getElementById('wordScanNext').disabled = _scanPage >= totalPages - 1;

  // Pager dots
  const pagerEl = document.getElementById('wordScanPager');
  if (totalPages <= 10) {
    pagerEl.innerHTML = Array.from({ length: totalPages }, (_, i) =>
      `<button class="wordscan-dot${i === _scanPage ? ' active' : ''}" onclick="_scanGoPage(${i})"></button>`
    ).join('');
  } else {
    pagerEl.innerHTML = '';
  }

  // Word grid — chỉ hiển thị từ, không có thông tin khác
  const grid = document.getElementById('wordScanGrid');
  grid.innerHTML = pageWords.map(w => {
    const isHidden = !!_scanDraft[w.word];
    return `<button class="wordscan-word-btn${isHidden ? ' hidden-word' : ''}"
      onclick="_scanToggleWord('${w.word.replace(/'/g, "\\'")}')"
      data-word="${w.word.replace(/"/g, '&quot;')}">
      ${h(w.word)}
      ${isHidden ? '<span class="wordscan-hidden-dot"></span>' : ''}
    </button>`;
  }).join('');
}

function _scanGoPage(page) {
  _scanPage = page;
  _renderScanPage();
}

function wordScanPrev() {
  if (_scanPage > 0) { _scanPage--; _renderScanPage(); }
}

function wordScanNext() {
  const totalPages = Math.ceil(words.length / SCAN_PAGE_SIZE);
  if (_scanPage < totalPages - 1) { _scanPage++; _renderScanPage(); }
}

function _scanToggleWord(word) {
  _scanDraft[word] = !_scanDraft[word];
  _renderScanPage();
}

async function saveWordScan() {
  // Áp dụng draft vào words[]
  let changed = false;
  words.forEach(w => {
    const newHidden = !!_scanDraft[w.word];
    if (w.hidden !== newHidden) {
      w.hidden = newHidden;
      if (newHidden) w.mastered = false; // reset mastered khi ẩn
      changed = true;
    }
  });
  closeWordScan();
  if (changed) {
    ...
    updateStudyBadge();
    refreshStatsBar();
    // Nếu đang ở tab flashcard thì re-render
    const fc = document.getElementById('panel-flashcard');
    if (fc && fc.classList.contains('active')) renderFlashcard();
    showToast('success', '✓ Đã lưu trạng thái rà soát!');
  }
}
