...
function _ltRenderExercise(groupIdx, exIdx) {
  ...
  if (!ex || !ex.length) {
    ...
    if (exIdx + 1 < _ltExOrder.length) _ltRenderExercise(groupIdx, exIdx + 1);
    else _ltRenderGroupResult(groupIdx);
    return;
  }
    ...
    <div class="lt-match">
      ...
      <div class="lt-match-area">
        <div class="lt-match-col" id="ltMatchWords">
          ${pairs.map((p,i) => `<div class="lt-match-item lt-match-word" data-idx="${i}" onclick="_ltMatchSelect(this,'word')">${p.word}</div>`).join('')}
        </div>
...
window._ltMatchSelect = function(el, type) {
  ...
  if (!state.selected) {
    ...
    const wordName  = wordEl.textContent.trim();
    const corrMeaning = document.querySelector(`[data-word="${wordName}"]`);

    if (corrMeaning === meaningEl) {
      wordEl.classList.add('matched');    meaningEl.classList.add('matched');
      wordEl.classList.remove('selected'); meaningEl.classList.remove('selected');
      state.correct++;
    } else {
      wordEl.classList.add('wrong-match');    meaningEl.classList.add('wrong-match');
      wordEl.classList.remove('selected');   meaningEl.classList.remove('selected');
      setTimeout(() => {
...
// Patch: next exercise khi exIdx = -1 → kết quả
const _ltOrigRenderEx = _ltRenderExercise;
function _ltRenderExercise(gi, ei) {
  if (ei === -1 || ei >= _ltExOrder.length) { _ltRenderGroupResult(gi); return; }
  _ltOrigRenderEx(gi, ei);
  // Init MCQ sau render
  const g = _ltGroups[gi];
  const exType = _ltExOrder[ei];
  if (exType === 'mcq' && g.exercises.mcq?.length) {
    setTimeout(() => _ltInitMCQ(g.exercises.mcq, gi, ei), 50);
  }
}
